...
class DataProcessing:
    ...
    def get_merged_table_as_dataframe_from_csv(self):
        ...
        merged = user_rated.merge(movie_genres, on='movie_id')
        merged.to_csv("merged.csv", sep='\t')

        ratings_one_hot = pd.concat([merged, pd.get_dummies(merged['genre'], prefix='genre')], axis=1)

        ratings_one_hot_grouped = ratings_one_hot.groupby(['user_id', 'movie_id', 'rating']).sum().drop(
    ...
    def get_user_avg_ratings_as_dict(self, ratings_dataframe, user_id):
        ...
        for x in avg_genre_ratings:
            avg_genre_ratings[x] = ratings_dataframe[(ratings_dataframe[x] == 1.0) & (ratings_dataframe['user_id'] == user_id)].loc[:, 'rating'].mean()

        # dodajemy user_id do slownika
        avg_genre_ratings['user_id'] = user_id
