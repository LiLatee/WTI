# {"user_id": 755,"movie_id": 3,"rating": 1,"genre_Adventure": null,"genre_Comedy": 1,"genre_Drama": null,"genre_Fantasy": null,"genre_Mystery": null,"genre_Romance": 1,"genre_Sci-Fi": null,"genre_Thriller": null,"genre_War": null}
import pandas as pd
import json
import numpy as np
import redis
...
from RedisCountAll import RedisCountAll
from RedisAvgAll_API import RedisAvgAll

from cassandra_.CassRatings_API import CassRatings
from cassandra_.CassProfiles_API import CassProfiles
from cassandra_.CassRatingsCount_API import CassRatingsCount
from cassandra_.CassCountAll_API import CassCountAll
from cassandra_.CassAvgAll_API import CassAvgAll
...
class API:
    def __init__(self):
        ...
        #
        # self.ratings = RedisRatings()
        # self.profiles = RedisProfiles()
        # self.count = RedisRatingsCount()
        # self.count_all = RedisCountAll()
        # self.avg_all = RedisAvgAll()

        self.ratings = CassRatings()
        self.profiles = CassProfiles()
        self.count = CassRatingsCount()
        self.count_all = CassCountAll()
        self.avg_all = CassAvgAll()

        self.fill_redis_from_csv()
        self.set_all_profiles_in_redis()
        self.set_all_avg_ratings_in_redis()
        self.set_all_count_of_ratings_in_redis()

    def fill_redis_from_csv(self):
        self.ratings.add_dataframe(self.data_processing.generate_ratings_as_dataframe_from_csv())
    ...
    def delete_all_ratings(self):
        ...
        self.count.delete_all()
        self.count_all.delete_all()
        self.avg_all.delete_all()
        return "deleted"

    def set_all_avg_ratings_in_redis(self):
        avg_all_as_dict = self.data_processing.get_all_avg_ratings_as_dict(self.ratings.get_all_as_dataframe())
        self.avg_all.set_avg_of_all_users(avg_all_as_dict)
    ...
    def set_all_profiles_in_redis(self):
        avg_all = self.data_processing.get_all_avg_ratings_as_dict(self.ratings.get_all_as_dataframe())
        users_ids_set = self.get_ids_of_all_users_as_set()

        for user_id in users_ids_set:
            avg_user = self.data_processing.get_user_avg_ratings_as_dict(self.ratings.get_all_as_dataframe(), user_id)
            ...
            self.profiles.set_profile(user_id=user_id, profile_dict=profile_dict)

    def set_all_count_of_ratings_in_redis(self):
        ratings_dataframe = self.ratings.get_all_as_dataframe()
        ratings_dataframe = ratings_dataframe.set_index(keys=['user_id', 'movie_id', 'rating'])
        ...
        for id in set_of_users_ids:
            ...
            self.count.set_count_for_user(user_id=id, genre_count_dict=count_of_ratings_dict)
    ...
    def get_ids_of_all_users_as_set(self):
        ...
        users_ids_set = set(ratings_dataframe['user_id'].values)
        return users_ids_set

    def get_all_avg_ratings_as_dict(self):
        # r = redis.StrictRedis(host='localhost', port=6381, db=0)
        # result = r.get('avg-all')
        return self.avg_all.get_avg_of_all_users_as_dict()
    ...
    def get_user_avg_ratings_as_dict(self, user_id, avg_all_dict=None):
        ...
        # print("get_user_avg_ratings_as_dict")
        if avg_all_dict is None:
            avg_all_dict = self.avg_all.get_avg_of_all_users_as_dict()
        user_profile = self.profiles.get_profile_as_dict(user_id=user_id)
        user_avg = {}
        for genre in avg_all_dict:
            try:
                user_avg[genre] = avg_all_dict[genre] - user_profile[genre]
            except KeyError:
                user_avg[genre] = avg_all_dict[genre]
    ...
    def get_user_profile_as_dict(self, user_id):
        return self.profiles.get_profile_as_dict(user_id)

    def get_count_of_ratings_of_all_genres_as_dict(self):
        return self.count_all.get_count_of_all_users_as_dict()

    def get_ratings_count_of_all_genres_for_user_as_dict(self, user_id):
        return self.count.get_count_of_user_as_dict(user_id=user_id)
    ...
    def update_avg_ratings_for_all_users_after_post(self, rating_dict):
        ...
        self.avg_all.set_avg_of_all_users(new_avg_all_dict)
    ...
    def update_count_of_ratings_after_post(self, rating_dict):
        user_id = rating_dict['user_id']
        old_genre_count_of_user_dict = self.count.get_count_of_user_as_dict(user_id=user_id)
        ...
        try:
            old_genre_count_of_all_users_dict = self.count_all.get_count_of_all_users_as_dict()
        except TypeError:
        ...
        self.count.set_count_for_user(user_id=user_id, genre_count_dict=new_genre_count_of_user_dict)
        self.count_all.set_count_of_all_users(genre_count_dict=new_genre_count_of_all_users_dict)
    ...
    #     genres_list_of_new_rating = list(rating_dict.keys())[3:-1]
    #
    #     r = redis_.StrictRedis(host='localhost', port=6381, db=0)
...
if __name__ == '__main__':
    ...
    r.post_rating(rating='{"user_id": 755,"movie_id": 3,"rating": 1,"genre_Adventure": null,"genre_Comedy": 1,"genre_Drama": null,"genre_Fantasy": null,"genre_Mystery": null,"genre_Romance": 1,"genre_Sci-Fi": null,"genre_Thriller": null,"genre_War": null}')
    ...
    # print(r.get_all_avg_ratings_as_dict())
    # rating_dict = json.loads('{"user_id": 75,"movie_id": 3,"rating": 1,"genre_Adventure": null,"genre_Comedy": 1,"genre_Drama": null,"genre_Fantasy": null,"genre_Mystery": null,"genre_Romance": 1,"genre_Sci-Fi": null,"genre_Thriller": null,"genre_War": null}')
    # print(rating_dict)
    ...
    # r.post_rating('{"user_id": 755,"movie_id": 3,"rating": 1,"genre_Adventure": null,"genre_Comedy": 1,"genre_Drama": null,"genre_Fantasy": null,"genre_Mystery": null,"genre_Romance": 1,"genre_Sci-Fi": null,"genre_Thriller": null,"genre_War": null}')
